...
| `list` | List of files/folders to upload | `FileItemType[]` | [] |
| `onSuccess` | Callback function when upload succeeds | `(fileTrees: DrawerResultFileTreeItemType[]) => void` | - |
| `isSelectFolder` | Whether to select a folder for download. If set to `false`, normal download will be used. If set to `true`, the user will be prompted to select a folder. If the browser does not support folder selection, normal download will also be used. | `boolean` | `true` |
...
<Playground code={FilesDownloadRaw} title='Download Files/Folders' description='Select folder download or normal download, and view the download effect.'>
  <FilesDownload />
</Playground>
